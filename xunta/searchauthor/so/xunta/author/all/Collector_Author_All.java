...
public class Collector_Author_All extends Collector {
	...
	public TreeSet<DocData_All> docdata_set=new TreeSet<DocData_All>(); //干脆直接用docdata排序,采用一个多载的创建方法,以避免创建docdata时获取暂时不需要的文档数据.
	...
	void docsAddup() throws Exception {
		//prt("测试搜索过程Collector_Author_All:"+current_Score+"|"+(docBase+current_DocId));
		...
		docdata_set.add(new DocData_All(docBase+current_DocId,current_Score,true));
