...
public class Collector_All extends Collector {
	...
	AuthorInfo_All new_ai;
	AuthorInfo_All old_ai;
	
	private HashMap<String,AuthorInfo_All> authorScoreMap=new HashMap<String,AuthorInfo_All>();// 作者ID与对应的作者信息类对象.用于遍历文章时积累各个作者的得分等数据.
	...
	public void collect(int docId) throws IOException{
		...
		prt("All作者:"+author+"\t"+current_Score);//这个打印中间加了制表符,可以拷到excel表格中作原始验证.
		...
		//int countingAuthors=0;//TODO 要删掉
			...
			*/
			...
			if (author.matches("doesNotMatchAnyCell_Xu")){//该作者名是无效情况下的记录,需过滤掉.//标题中的这种情况也应过滤掉.
				...
				new_ai=new AuthorInfo_All(author,site,current_Score);
				...
				//这个判断在最新搜索中不需要了：if (!old_ai.docIDs.containsValue(current_Score)){//如果该文档的分数原来不存在相同的分数,则累加.如果存在,则跳过累加.这是为了过滤掉相同的文档分数.
	...
	public HashMap<String, AuthorInfo_All> getAuthorScoreMap() {
