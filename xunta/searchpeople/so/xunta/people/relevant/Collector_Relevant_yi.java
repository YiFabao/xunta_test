...
public class Collector_Relevant_yi extends Collector {
	...
	public double getSumTime4() {
		return sumTime4;
	}

	public void setSumTime4(double sumTime4) {
		this.sumTime4 = sumTime4;
	}
	...
	public void setNextReader(AtomicReaderContext arc) throws IOException {
		...
		md5_DocBvalues=FieldCache.DEFAULT.getTerms(arc.reader(), "md5");//将md5调入内存
		long t2 = System.currentTimeMillis();
		sumTime1 += t2 - t1;// 统计执行setNextReader总用时
	...
	public void collect(int docId) throws IOException {
		...
		current_Score = current_Score + ((float) current_DocId) / 100000000;// 把id号附加到得分的尾部,解决得分相同时
																			// 排序不固定,无法对比校验的问题.
		// 在lucene41时用这两个:
		...
		String md5=md5_BytesRef.utf8ToString();//将二进制的doc值转成String
		...
		sumTime2 += t4 - t3;// 统计collector中获取具体作者名称，网站名称用时
		try {
			long t5 = System.currentTimeMillis();
			authorScoreAddup(md5);
	...
	void authorScoreAddup(String md5) throws Exception {
		...
		if (flag) {// 该作者名是无效情况下的记录,需过滤掉.//标题中的这种情况也应过滤掉.
			...
			new_ai = new AuthorInfo_Relevant(author, site, current_Score,md5);// 因为作者不存在，所以新产生一个作者时，第一个文档的得分就是作者的总得分
			...
			//判断当前的文档内容是否已经在作者的文档内容中出现
			if(!old_ai.md5Set.contains(md5))//文档不存在，说明文档是作者的非重复贴，该文档的分数要累加到作者的总分上
			{
				
				old_ai.totalScore+=current_Score;
				old_ai.md5Set.add(md5);
			}
		

			/*if (!old_ai.docIDs.containsValue(current_Score))//containsValue特别耗时
			{// 如果该文档的分数原来不存在相同的分数,则累加.如果存在,则跳过累加.这是为了过滤掉相同的文档分数.
				long t1 = System.currentTimeMillis();
