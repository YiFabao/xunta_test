...
import org.apache.lucene.search.Scorer; //import org.apache.lucene.search.FieldCache.DocTerms;//lucene41时用这个.
import org.apache.lucene.util.BytesRef;

import so.xunta.author.newest.*;

//2013年4月15日.
//2013.6.9:对于得分相同的文档,其得分将被忽略.
public class Collector_Newest extends Collector {
	...
	public double sumTime5 = 0;
	public double sumTime6=0;// 

	public double getSumTime1() {
		return sumTime1;
	}

	public void setSumTime1(double sumTime1) {
	...
	public void setSumTime2(double sumTime2) {
		this.sumTime2 = sumTime2;
	}

	public double getSumTime3() {
		return sumTime3;
	}

	public void setSumTime3(double sumTime3) {
	...
	private BinaryDocValues site_DocBValues;// lucene43
	...
	AuthorInfo_Newest new_ai;
	AuthorInfo_Newest old_ai;

	private HashMap<String, AuthorInfo_Newest> authorScoreMap = new HashMap<String, AuthorInfo_Newest>();// 作者ID与对应的作者信息类对象.用于遍历文章时积累各个作者的得分等数据.

	// private static TreeSet<AuthorInfo_small> authorRanking = new
	// TreeSet<AuthorInfo_small>();//
	// 最后的搜人数据容器.顺序是按总得分数排序的.重写了AuthorInfo的比较大小的方法.
	...
	/*
	 * 4.0版本以后使用下面的AtomicReaderContext方法参数. public void
	 * setNextReader(IndexReader reader, int docBase){ this.docBase=docBase; }
	 */
	public void setNextReader(AtomicReaderContext arc) throws IOException {
		this.arc = arc;
		this.docBase = arc.docBase; // docBase是通过arc传进来的.
		// lucene41用这个:authors_DocTerms=FieldCache.DEFAULT.
		// getTerms(arc.reader(),
		// "index_author");//作者名是得分累计时必须的//首次读入内存后可以一直使用.提高速度.
		// lucene41用这个:site_DocTerms=FieldCache.DEFAULT.getTerms(arc.reader(),
		// "index_sitename");//网站名称也是得分累计时必须的.作者需要用网站名来实现唯一化(不同网站的作者可能同名.//首次读入内存后可以一直使用.提高速度.
		...
		site_DocBValues = FieldCache.DEFAULT.getTerms(arc.reader(), "index_sitename");// 网站名称也是得分累计时必须的.作者需要用网站名来实现唯一化(不同网站的作者可能同名.//首次读入内存后可以一直使用.提高速度.
		timestamp_DocBvalues = FieldCache.DEFAULT.getTerms(arc.reader(), "index_publishtimestamp");
		long t2 = System.currentTimeMillis();
		sumTime1 += t2 - t1;

		// title_DocTerms=FieldCache.DEFAULT.getTerms(arc.reader(),
		// "index_title");//TODO 临时测试用
		// content_DocTerms=FieldCache.DEFAULT.getTerms(arc.reader(),
		// "index_content");//TODO 临时测试用
	...
	public void collect(int docId) throws IOException {
		...
		// 在lucene41时用这两个:
		// authors_DocTerms.getTerm(current_DocId,
		// author_BytesRef);//按当前的docId从内存中获得author字段的数据,通过bytesref参数返回.
		// site_DocTerms.getTerm(current_DocId,
		// site_BytesRef);//与上同.取出当前文档的网站名称.
		// 在lucene43时用这两个:
		long t3 = System.currentTimeMillis();
		authors_DocBValues.get(current_DocId, author_BytesRef);// 按当前的docId从内存中获得author字段的数据,通过bytesref参数返回.
		site_DocBValues.get(current_DocId, site_BytesRef);// 与上同.取出当前文档的网站名称.
		...
		// content_DocTerms.getTerm(current_DocId, content_BytesRef);//TODO
		// 临时测试用
		// String content=site_BytesRef.toString();//TODO 临时测试用
		// prt("-------------------------------current_DocId"+current_DocId);
		// prt("newest作者:"+author+"\t"+current_Score);//这个打印中间加了制表符,可以拷到excel表格中作原始验证.
	...
	void authorScoreAddup() throws Exception {
		...
		// DocData docData=new
		// DocData(scoreDoc,searcher);//生成一个含有必要文档信息的docData对象,并填入当前doc的数据.

		/*
		 * GroupingTest_Collector_Xu.end = System.currentTimeMillis();//TODO
		 * 要删掉. System.out.println("AuthorRanking-get(index_author)之前:" +
		 * (GroupingTest_Collector_Xu.end - GroupingTest_Collector_Xu.begin) +
		 * "ms"); //String
		 * author=searcher.doc(scoreDoc.doc).get("index_author"); String
		 * author="A+tmp"
		 * +countingDocs;//searcher.doc(scoreDoc.doc).get("index_author");
		 * GroupingTest_Collector_Xu.end = System.currentTimeMillis();//TODO
		 * 要删掉. System.out.println("AuthorRanking-get(index_author)之后:" +
		 * (GroupingTest_Collector_Xu.end - GroupingTest_Collector_Xu.begin) +
		 * "ms");
		 * 
		 * String
		 * title="title+amp";//searcher.doc(scoreDoc.doc).get("index_title");
		 * String
		 * site="site+amp";//searcher.doc(scoreDoc.doc).get("index_siteid");
		 * String authorId=author+"_"+site; countingDocs++;//TODO 要删掉
		 */
		totalDocs++;
		long t1=System.currentTimeMillis();
		boolean flag=author.equals("doesNotMatchAnyCell_Xu");
		long t2=System.currentTimeMillis();
		sumTime6+=t2-t1;
		if (flag) {// 该作者名是无效情况下的记录,需过滤掉.//标题中的这种情况也应过滤掉.
			// prt("作者名或标题中出现了 doesNotMatchAnyCell_Xu");
		} else if (!authorScoreMap.containsKey(authorPLUSsite)) {// 如果该作者不存在...
			new_ai = new AuthorInfo_Newest(author, site, current_Score);//占用时间几乎为0
			...
			authorScoreMap.put(authorPLUSsite, new_ai);// 将新作者放入authorScoreMap(作者名-AuthorInfo_small的map对.
			long t6=System.currentTimeMillis();
			sumTime5+=(t6-t5);										// 后者中有总得分.)
			...
			// 这个判断在最新搜索中不需要了：if
			// (!old_ai.docIDs.containsValue(current_Score)){//如果该文档的分数原来不存在相同的分数,则累加.如果存在,则跳过累加.这是为了过滤掉相同的文档分数.
			if (old_ai.totalScore < current_Score) {
				old_ai.totalScore = current_Score;// 如果该作者的新文档的得分(就是时间戳)大于原有的分数,则替换为新文档的分数.
			}
			long t6=System.currentTimeMillis();
			old_ai.docIDs.put(current_DocId + docBase, current_Score);// 将该文档的全局id和得分放入docID(一个docid-得分的map对:HashMap<Integer,Float>).
			long t7=System.currentTimeMillis();
			sumTime5+=t7-t6;
			// 上面的docid原来是hashmap. 不能保证顺序. 改成treemap. 2014.7.31
		}

		// prt("Collector_Xu - current_Score: "+current_Score);

		// prt("authorScoreAddup中的当前累计文档数-totalDocs:"+totalDocs+"个");//TODO 要删掉
		// prt("authorScoreAddup中的当前累计作者数-totalAuthors:"+
		// totalAuthors+"个");//TODO 要删掉

		// authorIdMap只是积累了作者名及其文档的所有数据,需要在数据转至AuthorRanking的过程中实现作者排序.
		// AuthorRanking的大小比较被重写,按得分来排序.
		// prt("从authorIdMap看作者总数:-----------authorIdMap.size():"+authorScoreMap.size()+"个");//TODO
		// 要删掉
	...
	public HashMap<String, AuthorInfo_Newest> getAuthorScoreMap() {
